...
describe("McpClient Break Session", () => {
    const itWithContext = (name, fn) => it(name, () => withTestContext({}, (ctx) => fn(ctx)));
    const createClient = (vscode) => {
    ...
    itWithContext("should successfully break a long-running forvalues loop", async ({ vscode }) => {
        const client = createClient(vscode);
    ...
    itWithContext("should allow sending a NEW command immediately after break_session and NOT cancel it", async ({ vscode }) => {
        const client = createClient(vscode);
